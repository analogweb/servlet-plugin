...
public class ForwardTest {
	...
	public void testRenderPathNotStartWithSlash() throws Exception {
		thrown.expect(new BaseMatcher<MissingRequirmentsException>() {
			@Override
			public boolean matches(Object arg0) {
				if (arg0 instanceof MissingRequirmentsException) {
					...
					assertThat(mr.getUnresolvableInvocationResult().toString(),
							is("foo/baa.rn"));
					return true;
				}
				return false;
	...
	public void testHashCode() throws Exception {
		assertThat(Forward.to("/foo/bar").hashCode(), is(Forward.to("/foo/bar")
				.hashCode()));
		assertThat(Forward.to("/foo/bar").hashCode(),
				is(not(Forward.to("/foo/bar2").hashCode())));
	}
	...
	public void testRenderWithContextObject() throws Exception {
		...
		doNothing().when(request).setAttribute("serializeable",
				extractToRequest);

		Forward.to(path).with(extractToRequest)
				.render(context, responseContext);
