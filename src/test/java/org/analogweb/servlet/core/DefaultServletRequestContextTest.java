...
public class DefaultServletRequestContextTest {
	...
	public void testGetCookies() {
		context = new DefaultServletRequestContext(request, response,
				servletContext);
	...
	public void testPutCookies() {
		...
		when(request.getMethod()).thenReturn("GET");
		when(request.getCookies()).thenReturn(
				new Cookie[]{new Cookie("hoge", "fuga")});
	...
	public void testGetContentType() {
		...
		when(request.getHeaders("Content-Type")).thenReturn(
				Collections.enumeration(Arrays.asList("text/xml",
						"application/xml")));
