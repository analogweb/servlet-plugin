...
public class DefaultServletRequestContext implements ServletRequestContext {
	...
	public DefaultServletRequestContext(HttpServletRequest request,
	...
	public RequestPath getRequestPath() {
		...
		return new DefaultRequestPath(URI.create(getServletContext()
				.getContextPath()), URI.create(request.getRequestURI()),
				request.getMethod());
	}
	...
	public Cookies getCookies() {
		if (this.cookies == null) {
			this.cookies = new ServletCookies(getServletRequest(),
					getServletResponse());
		}
		return this.cookies;
	}
	...
	static class ServletCookies implements Cookies {
		...
		public void putCookie(String name, Object value) {
			this.response.addCookie(new javax.servlet.http.Cookie(name, value
					.toString()));
		}
	...
	public Parameters getMatrixParameters() {
		if (this.matrixParameters == null) {
			this.matrixParameters = new MatrixParameters(
					URI.create(getServletRequest().getRequestURI()));
		}
	...
	public ReadableBuffer getRequestBody() throws IOException {
		return DefaultReadableBuffer.readBuffer(getServletRequest()
				.getInputStream());
	}

	@Override
	public MediaType getContentType() {
		List<String> contentTypes = getRequestHeaders().getValues(
				"Content-Type");
