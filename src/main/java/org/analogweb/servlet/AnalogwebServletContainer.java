...
/**
 * Analogweb {@link Application} running on Servlet environment. This container
 * describe as {@link Filter} or {@link HttpServlet}.
 * 
 * @author y2k2mt
 */
public class AnalogwebServletContainer extends HttpServlet implements Filter {
	...
	protected Response process(ServletRequest req, ServletResponse res)
		...
		try {
			Response proceeded = webApplication.processRequest(requestedPath,
			...
			if (entity instanceof DefaultResponseEntity) {
				DefaultWritableBuffer.writeBuffer(response.getOutputStream())
						.writeBytes(((DefaultResponseEntity) entity).entity());
			} else if (entity instanceof ReadableBufferResponseEntity) {
				DefaultWritableBuffer.writeBuffer(response.getOutputStream())
						.from(((ReadableBufferResponseEntity) entity).entity());
			}
			return proceeded;
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		this.webApplication.run(createApplicationContext(getServletContext()),
